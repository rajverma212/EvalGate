...
**Feature deletion.** `DELETE /api/features/{feature}` removes a feature outright: every run (with its `test_results` via the FK cascade), the regressions on either side of those runs, the baselines pointing at them, and then the feature's own bundle — spec, prompt versions, dataset versions. Implemented as `EvaluationStore.delete_feature`, one transaction. Unlike `delete_run` there is deliberately **no active-baseline guard**: a baseline exists to protect a feature's gate, and the whole feature is going away, so the confirmation belongs in the UI where the intent is expressed. After deletion the name is free to be created again from scratch.
...
**Filesystem-free activation (no schema change).** The web activation path no longer requires a writable platform root. `activate_feature_from_store(spec, cases, system_prompt, store, …)` builds the prompt/dataset definitions in memory, persists the spec/prompt/dataset to the database, rebuilds the registries *from the store*, runs the first evaluation through the unchanged engine, and persists the run — writing and reading **no bundle files**. The `POST /api/onboarding/activate` endpoint uses it, so online activation works on a read-only serverless filesystem — the root cause of the original "activated features vanish/404" problem.

**DB-only cutover (the database is now the runtime source of truth).** Discovery *and* evaluation resolve feature bundles from the database, not the filesystem. At startup each runtime calls `bootstrap_platform(store)` (in `activation/bootstrap.py`): it registers every feature — built-in *and* DB-activated (`register_installed_features(store=…)`) — and seeds the committed built-in bundles' content into the DB (`ensure_builtin_bundles`), which is idempotent because the content-hash upsert backfills only rows that lack content. The engine is then built from `load_prompts_from_store` / `load_datasets_from_store`, and the dashboard/API dataset explorer reads `dataset_versions.content`. The committed `prompts/` and `datasets/` directories are now only the **seed source** for the database, never a runtime resolution path. Consequences: an activated feature resolves in any fresh process (CLI, CI, a cold serverless instance) straight from the DB, so subsequent evaluations, regression detection, reports, and alerts all work with no filesystem and no extra configuration. The filesystem activation path (`write_feature_bundle` / `install_bundle` / `activate_bundle` / `run_first_evaluation`), the `specs/prompts/datasets` install dirs, `settings.platform_root`, and the legacy Streamlit onboarding wizard were removed — the Next.js *Create feature* flow is the single activation workflow.
...
> the existing onboarding/activation/evaluation cores — `activate_feature_from_store`
> (persist spec/prompt/dataset → rebuild registries from the store → unchanged engine →
> persist run) → `promote_baseline` — adding no new evaluation or persistence logic. It is
> **filesystem-free** (the database is the system of record) and needs `ANTHROPIC_API_KEY`;
> durable locally / on any persistent host, demo-grade on the ephemeral-`/tmp` serverless deploy.
